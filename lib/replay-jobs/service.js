...
function createReplayJobCancelledError(jobId) {
  return createServiceError(`Replay job "${jobId}" was cancelled`, {
    code: 'REPLAY_JOB_CANCELLED',
    statusCode: 409,
  });
}

function isReplayJobCancellationError(error) {
  return (
    error?.code === 'REPLAY_JOB_CANCELLED' ||
...
async function getReplayJob(jobId, { store } = {}) {
  if (!store) {
    throw createServiceError('Replay job store is required', {
      code: 'REPLAY_JOB_STORE_REQUIRED',
      statusCode: 500,
...
async function listReplayJobs({ limit = 50, store } = {}) {
  if (!store) {
    throw createServiceError('Replay job store is required', {
      code: 'REPLAY_JOB_STORE_REQUIRED',
      statusCode: 500,
...
async function cancelReplayJob(jobId, { now = () => new Date().toISOString(), store } = {}) {
...
async function assertReplayJobNotCancelled(jobId, { store } = {}) {
  const job = await getReplayJob(jobId, { store });

  if (job.status === JOB_STATUSES.CANCELLED) {
    throw createReplayJobCancelledError(jobId);
  }

  return job;
}

async function prepareReplayJobStart(
  jobId,
...
async function executeReplayJob(
  ...
  try {
    const summary = await replayRunner(
      {
        ...
        'job-id': job.jobId,
        ...(job.messagesPerSecond
          ? { 'messages-per-second': String(job.messagesPerSecond) }
          : {}),
      ...
      },
      {
        ...
        onProgress: async ({ lastReplayedOffset, replayedCount }) => {
          await assertReplayJobNotCancelled(jobId, { store });
    ...
    if (isReplayJobCancellationError(error)) {
      const currentJob = await getReplayJob(jobId, { store });
      const cancelledAt = currentJob.completedAt || completedAt;
...
async function startReplayJob(
  ...
  await prepareReplayJobStart(jobId, { now, store });
