const Database = require('better-sqlite3');
const fs = require('fs');
const path = require('path');

const JOB_STATUSES = Object.freeze({
  CANCELLED: 'cancelled',
  COMPLETED: 'completed',
  DRAFT: 'draft',
  FAILED: 'failed',
  RUNNING: 'running',
});

const DEFAULT_DB_PATH = path.join(
  /* turbopackIgnore: true */ process.cwd(),
  'data',
  'lighthouse.sqlite'
);

const COLUMN_MAP = {
  clientId: 'client_id',
  completedAt: 'completed_at',
  createdAt: 'created_at',
  destinationTopic: 'destination_topic',
  dryRun: 'dry_run',
  endOffset: 'end_offset',
  endTimestamp: 'end_timestamp',
  errorMessage: 'error_message',
  jobId: 'job_id',
  lastReplayedOffset: 'last_replayed_offset',
  messagesPerSecond: 'messages_per_second',
  partition: 'partition_id',
  progressInterval: 'progress_interval',
  progressTotal: 'progress_total',
  replayMode: 'replay_mode',
  replayedCount: 'replayed_count',
  sourceTopic: 'source_topic',
  startedAt: 'started_at',
  startOffset: 'start_offset',
  startTimestamp: 'start_timestamp',
  status: 'status',
  updatedAt: 'updated_at',
};

function resolveReplayDbPath(env = process.env) {
  return path.resolve(env.LIGHTHOUSE_DB_PATH || DEFAULT_DB_PATH);
}

function ensureParentDirectory(filePath) {
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
}

function toStoredValue(key, value) {
  if (value === undefined) {
    return value;
  }

  if (key === 'dryRun') {
    return value ? 1 : 0;
  }

  return value;
}

function normalizeCount(value) {
  const count = Number(value);

  if (!Number.isFinite(count) || count < 0) {
    return 0;
  }

  return Math.trunc(count);
}

function parseTimestampMs(value) {
  if (!value) {
    return null;
  }

  const timestamp = Date.parse(value);

  return Number.isFinite(timestamp) ? timestamp : null;
}

function roundMetric(value, decimalPlaces = 1) {
  return Number(value.toFixed(decimalPlaces));
}

function createProgressSnapshot(job) {
  const replayedCount = normalizeCount(job.replayedCount);
  const totalCount = normalizeCount(job.progressTotal);
  const countedReplay = Math.min(replayedCount, totalCount);
  const remainingCount = Math.max(totalCount - countedReplay, 0);
  const percent =
    totalCount > 0 ? roundMetric(Math.min((replayedCount / totalCount) * 100, 100)) : 0;
  const startedMs = parseTimestampMs(job.startedAt);
  const endedMs = parseTimestampMs(job.completedAt || job.updatedAt);
  const elapsedMs =
    startedMs !== null && endedMs !== null && endedMs >= startedMs
      ? endedMs - startedMs
      : null;
  const rawMessagesPerSecond =
    elapsedMs && elapsedMs > 0 ? countedReplay / (elapsedMs / 1000) : null;
  const averageMessagesPerSecond =
    rawMessagesPerSecond !== null ? roundMetric(rawMessagesPerSecond, 2) : null;
  const estimatedRemainingMs =
    job.status === JOB_STATUSES.RUNNING &&
    rawMessagesPerSecond &&
    rawMessagesPerSecond > 0
      ? Math.ceil((remainingCount / rawMessagesPerSecond) * 1000)
      : null;

  return {
    averageMessagesPerSecond,
    currentOffset: job.lastReplayedOffset,
    elapsedMs,
    estimatedRemainingMs,
    percent,
    remainingCount,
    replayedCount,
    totalCount,
  };
}

function mapJobRow(row) {
  if (!row) {
    return null;
  }

  const job = {
    clientId: row.client_id,
    completedAt: row.completed_at,
    createdAt: row.created_at,
    destinationTopic: row.destination_topic,
    dryRun: Boolean(row.dry_run),
    endOffset: row.end_offset,
    endTimestamp: row.end_timestamp,
    errorMessage: row.error_message,
    jobId: row.job_id,
    lastReplayedOffset: row.last_replayed_offset,
    messagesPerSecond: row.messages_per_second,
    partition: row.partition_id,
    progressInterval: row.progress_interval,
    progressTotal: row.progress_total,
    replayMode: row.replay_mode,
    replayedCount: row.replayed_count,
    sourceTopic: row.source_topic,
    startedAt: row.started_at,
    startOffset: row.start_offset,
    startTimestamp: row.start_timestamp,
    status: row.status,
    updatedAt: row.updated_at,
  };

  return {
    ...job,
    progress: createProgressSnapshot(job),
  };
}

function ensureReplayJobColumn(db, columnName, columnDefinition) {
  const columns = db.prepare('PRAGMA table_info(replay_jobs)').all();
  const columnNames = new Set(columns.map((column) => column.name));

  if (!columnNames.has(columnName)) {
    db.exec(`ALTER TABLE replay_jobs ADD COLUMN ${columnDefinition}`);
  }
}

function createReplayJobStore({
  dbPath = resolveReplayDbPath(),
  now = () => new Date().toISOString(),
} = {}) {
  ensureParentDirectory(dbPath);

  const db = new Database(dbPath);
  db.pragma('journal_mode = WAL');
  db.exec(`
    CREATE TABLE IF NOT EXISTS replay_jobs (
      job_id TEXT PRIMARY KEY,
      source_topic TEXT NOT NULL,
      destination_topic TEXT NOT NULL,
      partition_id INTEGER NOT NULL,
      start_offset INTEGER NOT NULL,
      end_offset INTEGER NOT NULL,
      status TEXT NOT NULL,
      replay_mode TEXT NOT NULL DEFAULT 'offset',
      start_timestamp TEXT,
      end_timestamp TEXT,
      messages_per_second INTEGER,
      dry_run INTEGER NOT NULL DEFAULT 0,
      replayed_count INTEGER NOT NULL DEFAULT 0,
      progress_total INTEGER NOT NULL,
      client_id TEXT NOT NULL,
      progress_interval INTEGER NOT NULL,
      created_at TEXT NOT NULL,
      started_at TEXT,
      completed_at TEXT,
      error_message TEXT,
      last_replayed_offset INTEGER,
      updated_at TEXT NOT NULL
    )
  `);
  ensureReplayJobColumn(db, 'replay_mode', "replay_mode TEXT NOT NULL DEFAULT 'offset'");
  ensureReplayJobColumn(db, 'start_timestamp', 'start_timestamp TEXT');
  ensureReplayJobColumn(db, 'end_timestamp', 'end_timestamp TEXT');
  ensureReplayJobColumn(db, 'messages_per_second', 'messages_per_second INTEGER');

  const selectJobStatement = db.prepare(`
    SELECT
      job_id,
      source_topic,
      destination_topic,
      partition_id,
      start_offset,
      end_offset,
      status,
      replay_mode,
      start_timestamp,
      end_timestamp,
      messages_per_second,
      dry_run,
      replayed_count,
      progress_total,
      client_id,
      progress_interval,
      created_at,
      started_at,
      completed_at,
      error_message,
      last_replayed_offset,
      updated_at
    FROM replay_jobs
    WHERE job_id = ?
  `);
  const listJobsStatement = db.prepare(`
    SELECT
      job_id,
      source_topic,
      destination_topic,
      partition_id,
      start_offset,
      end_offset,
      status,
      replay_mode,
      start_timestamp,
      end_timestamp,
      messages_per_second,
      dry_run,
      replayed_count,
      progress_total,
      client_id,
      progress_interval,
      created_at,
      started_at,
      completed_at,
      error_message,
      last_replayed_offset,
      updated_at
    FROM replay_jobs
    ORDER BY created_at DESC, job_id DESC
    LIMIT ?
  `);
  const insertJobStatement = db.prepare(`
    INSERT INTO replay_jobs (
      job_id,
      source_topic,
      destination_topic,
      partition_id,
      start_offset,
      end_offset,
      status,
      replay_mode,
      start_timestamp,
      end_timestamp,
      messages_per_second,
      dry_run,
      replayed_count,
      progress_total,
      client_id,
      progress_interval,
      created_at,
      started_at,
      completed_at,
      error_message,
      last_replayed_offset,
      updated_at
    ) VALUES (
      @jobId,
      @sourceTopic,
      @destinationTopic,
      @partition,
      @startOffset,
      @endOffset,
      @status,
      @replayMode,
      @startTimestamp,
      @endTimestamp,
      @messagesPerSecond,
      @dryRun,
      @replayedCount,
      @progressTotal,
      @clientId,
      @progressInterval,
      @createdAt,
      @startedAt,
      @completedAt,
      @errorMessage,
      @lastReplayedOffset,
      @updatedAt
    )
  `);

  function getJob(jobId) {
    return mapJobRow(selectJobStatement.get(jobId));
  }

  function listJobs({ limit = 50 } = {}) {
    return listJobsStatement.all(limit).map(mapJobRow);
  }

  function createJob(job) {
    insertJobStatement.run({
      endTimestamp: null,
      messagesPerSecond: null,
      replayMode: 'offset',
      startTimestamp: null,
      ...job,
      dryRun: job.dryRun ? 1 : 0,
    });

    return getJob(job.jobId);
  }

  function updateJob(jobId, updates) {
    const entries = Object.entries(updates).filter(([, value]) => value !== undefined);

    if (entries.length === 0) {
      return getJob(jobId);
    }

    const parameters = { jobId };
    const assignments = entries.map(([key, value]) => {
      const columnName = COLUMN_MAP[key];

      if (!columnName) {
        throw new Error(`Unknown replay job field "${key}"`);
      }

      parameters[key] = toStoredValue(key, value);
      return `${columnName} = @${key}`;
    });

    if (!Object.prototype.hasOwnProperty.call(updates, 'updatedAt')) {
      parameters.updatedAt = now();
      assignments.push('updated_at = @updatedAt');
    }

    db.prepare(
      `UPDATE replay_jobs SET ${assignments.join(', ')} WHERE job_id = @jobId`
    ).run(parameters);

    return getJob(jobId);
  }

  function close() {
    db.close();
  }

  return {
    close,
    createJob,
    dbPath,
    getJob,
    listJobs,
    updateJob,
  };
}

module.exports = {
  COLUMN_MAP,
  DEFAULT_DB_PATH,
  JOB_STATUSES,
  createProgressSnapshot,
  createReplayJobStore,
  mapJobRow,
  resolveReplayDbPath,
};
