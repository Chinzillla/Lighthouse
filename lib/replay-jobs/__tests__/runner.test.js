/** @jest-environment node */

const fs = require('fs');
const os = require('os');
const path = require('path');
const { createReplayJobStore, JOB_STATUSES } = require('../store');
const { createReplayJob } = require('../service');
const { createInMemoryReplayJobRunner } = require('../runner');

function createTempStore() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lighthouse-replay-runner-'));
  const dbPath = path.join(tempDir, 'jobs.sqlite');
  const store = createReplayJobStore({ dbPath });

  return {
    cleanup: () => {
      store.close();
      fs.rmSync(tempDir, { force: true, recursive: true });
    },
    dbPath,
    store,
  };
}

describe('Replay job runner', () => {
  it('starts jobs in the background and persists the completed result', async () => {
    const { cleanup, dbPath, store } = createTempStore();

    try {
      await createReplayJob(
        {
          destination: 'orders-replay',
          end: '12',
          'job-id': 'job-background',
          partition: '0',
          source: 'orders',
          start: '11',
        },
        {
          env: {
            KAFKA_BROKERS: 'localhost:19092',
          },
          now: () => '2026-04-28T16:00:00.000Z',
          store,
        }
      );

      const runner = createInMemoryReplayJobRunner();
      const runningJob = await runner.start('job-background', {
        env: {
          KAFKA_BROKERS: 'localhost:19092',
          LIGHTHOUSE_DB_PATH: dbPath,
        },
        now: () => '2026-04-28T16:05:00.000Z',
        replayRunner: async (_options, { onProgress }) => {
          await onProgress({ lastReplayedOffset: 11, replayedCount: 1 });
          await onProgress({ lastReplayedOffset: 12, replayedCount: 2 });

          return {
            lastReplayedOffset: 12,
            replayedCount: 2,
          };
        },
      });

      expect(runningJob).toMatchObject({
        jobId: 'job-background',
        startedAt: '2026-04-28T16:05:00.000Z',
        status: JOB_STATUSES.RUNNING,
      });
      expect(runner.isRunning('job-background')).toBe(true);

      await runner.waitForJob('job-background');

      const completedJob = store.getJob('job-background');
      expect(completedJob).toMatchObject({
        completedAt: '2026-04-28T16:05:00.000Z',
        lastReplayedOffset: 12,
        replayedCount: 2,
        status: JOB_STATUSES.COMPLETED,
      });
      expect(runner.isRunning('job-background')).toBe(false);
    } finally {
      cleanup();
    }
  });

  it('cancels active background jobs and aborts the replay runner', async () => {
    const { cleanup, dbPath, store } = createTempStore();
    let replaySignal;
    let resolveRunnerStarted;
    const runnerStarted = new Promise((resolve) => {
      resolveRunnerStarted = resolve;
    });

    try {
      await createReplayJob(
        {
          destination: 'orders-replay',
          end: '12',
          'job-id': 'job-background-cancel',
          partition: '0',
          source: 'orders',
          start: '11',
        },
        {
          env: {
            KAFKA_BROKERS: 'localhost:19092',
          },
          now: () => '2026-04-28T16:10:00.000Z',
          store,
        }
      );

      const runner = createInMemoryReplayJobRunner();
      await runner.start('job-background-cancel', {
        env: {
          KAFKA_BROKERS: 'localhost:19092',
          LIGHTHOUSE_DB_PATH: dbPath,
        },
        now: () => '2026-04-28T16:15:00.000Z',
        replayRunner: async (_options, { signal }) => {
          replaySignal = signal;
          resolveRunnerStarted();

          return new Promise((_resolve, reject) => {
            signal.addEventListener('abort', () => reject(signal.reason), {
              once: true,
            });
          });
        },
      });

      await runnerStarted;

      const cancelledJob = await runner.cancel('job-background-cancel', {
        env: {
          LIGHTHOUSE_DB_PATH: dbPath,
        },
        now: () => '2026-04-28T16:16:00.000Z',
      });

      expect(cancelledJob).toMatchObject({
        completedAt: '2026-04-28T16:16:00.000Z',
        jobId: 'job-background-cancel',
        status: JOB_STATUSES.CANCELLED,
      });
      expect(replaySignal.aborted).toBe(true);

      await runner.waitForJob('job-background-cancel');

      expect(store.getJob('job-background-cancel')).toMatchObject({
        completedAt: '2026-04-28T16:16:00.000Z',
        errorMessage: null,
        status: JOB_STATUSES.CANCELLED,
      });
      expect(runner.isRunning('job-background-cancel')).toBe(false);
    } finally {
      cleanup();
    }
  });
});
