...
describe('Replay job service', () => {
  it('creates draft jobs from replay options and lists them', async () => {
    ...
    try {
      ...
      await expect(listReplayJobs({ store })).resolves.toEqual([job]);
    } finally {
      cleanup();
    }
  });
  ...
  it('marks running jobs cancelled when cancellation is requested during replay', async () => {
    const { cleanup, store } = createTempStore();
    ...
    try {
      await createReplayJob(
        ...
        },
        {
          ...
          now: () => '2026-04-28T13:56:00.000Z',
          store,
        }
      );

      const updatedJob = await startReplayJob('job-cancel-running', {
        env: {
          KAFKA_BROKERS: 'localhost:19092',
        },
        now: () => '2026-04-28T13:57:00.000Z',
        replayRunner: async (_options, { onProgress }) => {
          await onProgress({ lastReplayedOffset: 11, replayedCount: 1 });
          await cancelReplayJob('job-cancel-running', {
            now: () => '2026-04-28T13:57:05.000Z',
            store,
          });
          await onProgress({ lastReplayedOffset: 12, replayedCount: 2 });
      ...
      expect(updatedJob).toMatchObject({
        completedAt: '2026-04-28T13:57:05.000Z',
        errorMessage: null,
  ...
  it('marks running jobs cancelled when cancellation is requested after the last progress update', async () => {
    const { cleanup, store } = createTempStore();
    ...
    try {
      await createReplayJob(
        {
          ...
          'job-id': 'job-cancel-after-progress',
          partition: '0',
          source: 'orders',
          start: '11',
        },
      ...
      const updatedJob = await startReplayJob('job-cancel-after-progress', {
        env: {
          KAFKA_BROKERS: 'localhost:19092',
        },
        now: () => '2026-04-28T13:57:00.000Z',
        replayRunner: async (_options, { onProgress }) => {
          await onProgress({ lastReplayedOffset: 12, replayedCount: 2 });
          // Cancellation is persisted after the last progress update but before the runner returns
          await cancelReplayJob('job-cancel-after-progress', {
            now: () => '2026-04-28T13:57:05.000Z',
            store,
          });
      ...
      expect(updatedJob).toMatchObject({
        completedAt: '2026-04-28T13:57:05.000Z',
        errorMessage: null,
        jobId: 'job-cancel-after-progress',
        status: JOB_STATUSES.CANCELLED,
      });
    } finally {
      cleanup();
  ...
  it('rejects invalid replay job list limits', async () => {
    const { cleanup, store } = createTempStore();

    try {
      await expect(listReplayJobs({ limit: 0, store })).rejects.toThrow(
