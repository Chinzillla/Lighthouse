...
async function requestJson(url, { body, fetcher = globalThis.fetch, method = 'GET' } = {}) {
...
function fetchReplayJobs({ fetcher, limit = RECENT_JOB_LIMIT } = {}) {
  return requestJson(`/api/jobs?limit=${limit}`, { fetcher });
}

function createReplayJobDraft(input, { fetcher } = {}) {
