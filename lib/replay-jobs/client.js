...
function parseOptionalPositiveInteger(label, value) {
  const normalizedValue = String(value ?? '').trim();
  ...
  if (!/^\d+$/.test(normalizedValue) || Number(normalizedValue) < 1) {
    throw createClientError(`${label} must be a positive integer`, 400);
  }

  return normalizedValue;
}

function parseRequiredTimestamp(label, value) {
  const normalizedValue = String(value || '').trim();

  if (!normalizedValue) {
    throw createClientError(`${label} must be a valid timestamp`, 400);
  }

  let timestampMs;

  if (/^\d+$/.test(normalizedValue)) {
    timestampMs = Number(normalizedValue);

    if (!Number.isSafeInteger(timestampMs) || timestampMs < 0) {
      throw createClientError(`${label} must be a valid timestamp`, 400);
    }
  } else {
    timestampMs = Date.parse(normalizedValue);
...
function parseReplayMode(value) {
  if (value === 'timestamp') {
    return 'timestamp';
  }
...
function validateReplayDraftInput(input) {
  ...
  const messagesPerSecond = parseOptionalPositiveInteger(
    'Max messages per second',
    input.messagesPerSecond
  );
  const replayMode = parseReplayMode(input.mode);
  ...
  if (replayMode === 'timestamp') {
    const startTimestamp = parseRequiredTimestamp('Start timestamp', input.startTimestamp);
    const endTimestamp = parseRequiredTimestamp('End timestamp', input.endTimestamp);

    if (Date.parse(startTimestamp) >= Date.parse(endTimestamp)) {
      throw createClientError('Start timestamp must be before end timestamp', 400);
    }
    ...
    return {
      ...
      ...(messagesPerSecond ? { 'messages-per-second': messagesPerSecond } : {}),
      ...(jobId ? { 'job-id': jobId } : {}),
    };
  }
