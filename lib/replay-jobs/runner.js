const { createReplayJobStoreFromEnv } = require('./store-factory');
const {
  createConflictError,
  cancelReplayJob,
...
function createInMemoryReplayJobRunner() {
  ...
  async function start(
    ...
    const store = createReplayJobStoreFromEnv({ env, now });
    const controller = new AbortController();

    let runningJob;

    try {
      runningJob = await prepareReplayJobStart(jobId, { now, store });
    } catch (error) {
      await store.close();
      throw error;
    }
  ...
  async function cancel(
    ...
    try {
      const cancelledJob = await cancelReplayJob(jobId, { now, store });
